...
from mjai.mlibriichi.arena import MultiMatch  # type: ignore
...
class MultiSimulator:
    def __init__(
        ...
        timeout: float = 2.0,
        port: int = 28090,
        seeds: list[tuple[int, int]] = [(10000, 2000), (10000, 2000), (10001, 2001)],
    ):
    ...
    def run(self, dup_index: int = 0):
        ...
        for run_idx in range(max_run):
            ...
            if next_state["cur_idx"] == len(self.seeds):
                break
    ...
    def get_next_state(self, jsonl_path: Path | str, cur_idx: int) -> dict | None:
    ...
    def _internal_run(
        self, run_idx: int = 0, next_state: dict | None = None # next_state should be named prev_state
    ) -> dict | None:
        next_state_after_run = None

        with tempfile.TemporaryDirectory() as dirpath:
            env = MultiMatch(log_dir=dirpath)
            ...
            try:
                logger.info(f"Start hanchang games.")
                ...
                elif not next_state["continue_game"]:
                    env.py_match(
                        agents[0],
                        agents[1],
                        agents[2],
            ...
            for game_json in game_jsons:
                ...
                # JSONL ログを保存
                (self.logs_dir / f"{run_idx}.mjai.jsonl").open("a").write(
                    "\n".join(
                        [
            ...
            # JSONL ログから終了判定
            cur_idx = 0 if next_state is None else next_state["cur_idx"]
            next_state_after_run = self.get_next_state(
                self.logs_dir / f"{run_idx}.mjai.jsonl", cur_idx,
            )
